...
def main() -> None:
    ...
    parser.add_argument("foreign_access_followup_json")
    parser.add_argument("public_market_review_surface_json")
    parser.add_argument("public_market_pick_json")
    parser.add_argument("--output-json", required=True)
    args = parser.parse_args()

    output = build_public_market_foreign_review_handoff(
        json.loads(Path(args.foreign_access_followup_json).read_text(encoding="utf-8")),
