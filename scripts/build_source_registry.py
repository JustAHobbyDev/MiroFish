#!/usr/bin/env python3
"""
Build a structured source registry from the current source-priority docs.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional override for source-priority matrix doc",
    )
    parser.add_argument(
        "--investigation-path",
        type=Path,
        default=None,
        help="Optional override for source investigation list doc",
    )
    parser.add_argument("--output-json", required=True, type=Path)
    args = parser.parse_args()

    module = _load_source_registry_module()
    payload = module.build_source_registry_from_docs(
