#!/usr/bin/env python3
"""
Build the first deterministic bounded-entity expansion batch.
"""
...
def main() -> int:
    ...
    parser.add_argument("family_batch_json", type=Path)
    parser.add_argument("assessment_json", type=Path)
    parser.add_argument("--output-json", type=Path, required=True)
    args = parser.parse_args()

    module = _load_module("bounded_entity_expander")
    payload = module.build_bounded_entity_expansion_batch(
        json.loads(args.family_batch_json.read_text(encoding="utf-8")),
        json.loads(args.assessment_json.read_text(encoding="utf-8")),
    )
