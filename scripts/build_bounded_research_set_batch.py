#!/usr/bin/env python3
"""
Build bounded research sets from expansion plans and one or more prefilter batches.
"""
...
def main() -> int:
    ...
    parser.add_argument("expansion_batch_json", type=Path)
    parser.add_argument("prefilter_batch_json", nargs="+", type=Path)
    parser.add_argument("--output-json", type=Path, required=True)
    args = parser.parse_args()

    module = _load_module("bounded_research_set_builder")
    payload = module.build_bounded_research_set_batch(
        _load_json(args.expansion_batch_json),
